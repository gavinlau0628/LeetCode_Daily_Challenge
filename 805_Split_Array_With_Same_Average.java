class Solution {    
    public boolean checker(int[] A, int leftSum, int leftNum, int startIndex) {       
        if (leftNum == 0) 
            return leftSum == 0;
        ...
        for (int i = startIndex; i < A.length - leftNum + 1; i ++) 
        {
	        if (i > startIndex && A[i] == A[i - 1]) continue;
                if (checker(A, leftSum - A[i], leftNum - 1, i + 1)) 
                    return true;
        }
        return false;       
    }
    ...
    public boolean splitArraySameAverage(int[] A) {
        ...
        for (int lenOfB = 1; lenOfB <= A.length / 2; lenOfB ++) {
            if ((sumA * lenOfB) % A.length == 0) {
                if (checker(A, (sumA * lenOfB) / A.length, lenOfB, 0)) return true;
            }
        }
        return false;
